...
const fetchLookups = async () => {
  ...
  try {
    const { data } = await axios.get("/api/delivery-notes/create");
...
const submit = async () => {
  ...
  try {
    ...
    if (err.response?.status === 422) {
      ...
      errors.server = "Une erreur est survenue lors de l'enregistrement du bon de livraison.";
          ...
          <form v-else @submit.prevent="submit" class="p-6 lg:p-8 space-y-8">
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                <h3 class="text-sm font-bold text-[#062121] uppercase tracking-wider">Lignes du bon de livraison</h3>
                <button
                  v-if="!isFromQuote"
                  type="button"
                  @click="addLine"
