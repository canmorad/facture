        ...
        <div class="rounded-2xl border border-gray-200 bg-[#F4F7F7] shadow-sm overflow-hidden">
          ...
          <div class="border-b border-gray-200 px-6 pt-4 pb-3">
            <div class="flex gap-6">
              ...
              >
                ...
                {{ editingId ? 'Modifier l\'intérêt' : 'Ajouter un intérêt' }}
              </button>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'list'" class="p-6 lg:p-8">
            <div v-if="isLoading" class="text-center py-12">
              ...
              <p class="mt-2 text-gray-500">Chargement des intérêts...</p>
            </div>

            <div v-else-if="items.length === 0" class="text-center py-12">
              <i class="fas fa-clock text-5xl text-gray-300 mb-4 block"></i>
              <p class="text-gray-500">Aucun intérêt de retard enregistré.</p>
              <button
                @click="changeTab('add')"
              ...
              >
                <i class="fas fa-plus"></i> Ajouter votre premier intérêt
              </button>
            </div>
...
const baseUrl = computed(() => `/api/companies/${authStore.currentCompanyId}/late-fee-interests`);
...
const fetchItems = async () => {
  ...
  try {
    ...
    error("Erreur", "Impossible de charger les intérêts de retard.");
  } finally {
...
const submit = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      ...
      success("Modifié !", "L'intérêt a été modifié.");
    } else {
      await axios.post(baseUrl.value, payload);
      success("Ajouté !", "L'intérêt a été ajouté.");
    }
    resetForm();
    await fetchItems();
...
const toggleActive = async (item) => {
  try {
    ...
    success("Statut mis à jour", `L'intérêt est maintenant ${item.is_active ? 'actif' : 'inactif'}.`);
  } catch {
...
const setDefault = async (item) => {
  try {
    ...
    success("Défaut", "Cet intérêt est maintenant la valeur par défaut.");
  } catch {
    error("Erreur", "Impossible de définir cet intérêt par défaut.");
  }
};
...
const deleteItem = async (item) => {
  ...
  try {
    ...
    success("Supprimé !", "L'intérêt a été supprimé.");
  } catch {
    error("Erreur", "Impossible de supprimer l'intérêt.");
  }
};
