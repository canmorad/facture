    ...
    <div class="bg-gray-50 min-h-screen font-sans antialiased">
      <div class="p-6 lg:p-8">
        <div class="rounded-2xl border border-gray-200 bg-[#F4F7F7] shadow-sm overflow-hidden">
          ...
          <div v-else-if="activeTab === 'pending'" class="p-6 lg:p-8">
            <div v-if="isLoading" class="text-center py-12">
              <svg class="animate-spin h-8 w-8 mx-auto text-[#C5F82A]" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              ...
              <table class="min-w-full">
                <thead>
                  <tr class="border-b border-gray-200">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-[#062121] uppercase tracking-wider">Rôle</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-[#062121] uppercase tracking-wider">Expire</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-[#062121] uppercase tracking-wider">Statut</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                  ...
                  >
                    <td class="px-4 py-4">
                      <div class="flex items-center gap-3">
                        <div class="h-10 w-10 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-sm">
                          <i class="fas fa-envelope"></i>
                        </div>
                        <div>
                          <div class="text-sm font-semibold text-gray-900">{{ invitation.email }}</div>
                        </div>
                      </div>
                    </td>
                    <td class="px-4 py-4">
                      <span
                        :class="[
                          ...
                          invitation.role === 'accountant' ? 'bg-purple-100 text-purple-800' :
                          invitation.role === 'assistant-accountant' ? 'bg-teal-100 text-teal-800' :
                          'bg-gray-100 text-gray-600'
                        ]"
                      >
                    ...
                    <td class="px-4 py-4">
                      <span class="text-xs text-gray-500">{{ formatDate(invitation.expires_at) }}</span>
                    </td>
                    <td class="px-4 py-4">
...
const submitInvite = async () => {
  ...
  try {
    ...
    const { data } = await axios.post('/api/company/users/invite', payload)
    if (data.user.pending) {
      pendingInvitations.value.push(data.user)
      success('Invitation envoyée', `Un email d'invitation a été envoyé à ${inviteForm.email}.`)
      changeTab('pending')
    } else {
      users.value.push(data.user)
      success('Utilisateur ajouté', `${data.user.email} a été ajouté à l'organisation.`)
      changeTab('list')
    }
  } catch (err) {
