        ...
        <div class="rounded-2xl border border-gray-200 bg-[#F4F7F7] shadow-sm overflow-hidden">
          ...
          <div class="border-b border-gray-200 px-6 pt-4 pb-3">
            <div class="flex gap-6">
              ...
              >
                ...
                {{ editingId ? 'Modifier la condition' : 'Ajouter une condition' }}
              </button>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'list'" class="p-6 lg:p-8">
            <div v-if="isLoading" class="text-center py-12">
              ...
              <p class="mt-2 text-gray-500">Chargement des conditions...</p>
            </div>

            <div v-else-if="items.length === 0" class="text-center py-12">
              <i class="fas fa-file-invoice text-5xl text-gray-300 mb-4 block"></i>
              <p class="text-gray-500">Aucune condition de paiement enregistrée.</p>
              <button
                @click="changeTab('add')"
              ...
              >
                <i class="fas fa-plus"></i> Ajouter votre première condition
              </button>
            </div>
...
const baseUrl = computed(() => {
  return `/api/companies/${authStore.currentCompanyId}/payment-conditions`;
});
...
const fetchItems = async () => {
  ...
  try {
    ...
    error("Erreur", "Impossible de charger les conditions.");
  } finally {
...
const submit = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      ...
      success("Modifié !", "La condition a été modifiée.");
    } else {
      await axios.post(baseUrl.value, payload);
      success("Ajouté !", "La condition a été ajoutée.");
    }
    resetForm();
    await fetchItems();
...
const toggleActive = async (item) => {
  try {
    ...
    success("Statut mis à jour", `La condition est maintenant ${item.is_active ? 'active' : 'inactive'}.`);
  } catch {
...
const setDefault = async (item) => {
  try {
    ...
    success("Défaut", "Cette condition est maintenant la valeur par défaut.");
  } catch {
    error("Erreur", "Impossible de définir cette condition par défaut.");
  }
};
...
const deleteItem = async (item) => {
  ...
  try {
    ...
    success("Supprimé !", "La condition a été supprimée.");
  } catch {
    error("Erreur", "Impossible de supprimer la condition.");
  }
};
