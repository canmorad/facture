<!-- src/views/proformas/CreateProforma.vue -->
<script setup>
...
const fetchLookups = async () => {
  ...
  try {
    const { data } = await axios.get("/api/proformas/create");
...
const fetchDocument = async (documentId) => {
  isLoading.value = true;
  try {
    const { data } = await axios.get(`/api/proformas/${documentId}`);
    ...
    form.validity_date = doc.validity_date || "";
...
const handleSelectProduct = ({ index, product }) => {
  ...
  // Récupérer l'ID de la catégorie du produit (category_id, pas product_category_id)
  let productTypeId = "";
...
const submit = async () => {
  ...
  const confirmed = await confirm(
    isEditMode.value ? "Mettre à jour la facture proforma" : "Enregistrer la facture proforma",
    isEditMode.value
      ? "Les modifications seront enregistrées."
      : "La facture proforma sera enregistrée en tant que brouillon. Vous pourrez la finaliser ultérieurement.",
  );
  ...
  try {
    ...
    if (isEditMode.value) {
      await axios.put(`/api/proformas/${route.params.id}`, payload);
      success("Facture proforma mise à jour", "La facture proforma a été mise à jour avec succès.");
    } else {
      await axios.post("/api/proformas", payload);
      success("Facture proforma enregistrée", "La facture proforma a été enregistrée en tant que brouillon.");
    }
    router.push("/proformas");
  } catch (err) {
    if (err.response?.status === 422) {
      ...
      errors.server =
        "Une erreur est survenue lors de l'enregistrement de la facture proforma.";
...
const saveAndFinalize = async () => {
  ...
  const confirmed = await confirm(
    isEditMode.value ? "Finaliser les modifications" : "Finaliser la facture proforma",
    isEditMode.value
      ? "La facture proforma sera finalisée avec les modifications."
      : "La facture proforma sera finalisée et un numéro lui sera attribué.",
  );
  if (!confirmed.isConfirmed) return;
  ...
  try {
    ...
    if (isEditMode.value) {
      await axios.put(`/api/proformas/${route.params.id}`, payload);
      await axios.put(`/api/proformas/${route.params.id}/finalize`);
      success("Finalisé !", "La facture proforma a été mise à jour et finalisée.");
    } else {
      const response = await axios.post("/api/proformas", payload);
      const createdDoc = response.data;
      await axios.put(`/api/proformas/${createdDoc.id}/finalize`);
      success("Finalisé !", `La facture proforma ${createdDoc.number} a été finalisée.`);
    }
    router.push("/proformas");
  } catch (err) {
    if (err.response?.status === 422) {
