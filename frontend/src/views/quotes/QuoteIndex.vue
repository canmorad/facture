...
const fetchDevis = async () => {
  ...
  try {
    ...
    const message = err.response?.data?.error || "Impossible de charger les devis.";
...
const filteredDevis = computed(() => {
  ...
  return devis.value.filter(d => d.documentable?.status === selectedStatus.value);
});
...
const toggleDropdown = (id, event) => {
  ...
  // Positionner le dropdown sous le bouton, aligné à gauche du bouton
  // Si le bouton est à droite, on décale le dropdown vers la gauche de sa largeur
  const dropdownWidth = 240; // largeur approximative en pixels
  const windowWidth = window.innerWidth;
  
  let left = rect.left;
  // Si le dropdown dépasse à droite, on le décale vers la gauche
  if (left + dropdownWidth > windowWidth - 10) {
    left = windowWidth - dropdownWidth - 10;
  }
  // Si le dropdown dépasse à gauche, on le met à 10px du bord gauche
  if (left < 10) {
    left = 10;
  }
...
const deleteDevis = async (id, number) => {
  ...
  try {
    ...
    const message = err.response?.data?.error || "Impossible de supprimer le devis.";
    error("Erreur", message);
  }
};
...
const finalizeDevis = async (id) => {
  ...
  try {
    const companyId = authStore.currentCompanyId;
    const { data } = await axios.put(`/api/quotes/${id}/finalize`, null, {
      params: { company_id: companyId },
    });
    success("Finalisé !", `Le devis ${data.number} a été finalisé.`);
    await fetchDevis();
  } catch (err) {
    const message = err.response?.data?.error || "Impossible de finaliser le devis.";
    error("Erreur", message);
  }
};
...
const convertToInvoice = async (quote) => {
  ...
  try {
    const companyId = authStore.currentCompanyId;
    const { data } = await axios.post(`/api/quotes/${quote.id}/convert-to-invoice`, null, {
      params: { company_id: companyId },
    });
    success("Converti !", `Le devis ${quote.number} est maintenant la facture ${data.number}.`);
    await fetchDevis();
    router.push("/invoices");
  } catch (err) {
    const message = err.response?.data?.error || "Impossible de convertir le devis.";
    error("Erreur", message);
  }
};
      ...
      <div class="p-6 lg:p-8">
        <div class="rounded-2xl border border-gray-200 bg-[#F4F7F7] shadow-sm overflow-hidden">
            ...
            <div class="flex items-center justify-between flex-wrap gap-4">
              <div class="flex gap-6">
                ...
                >
                  ...
                  <span class="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">
                    {{ devis.filter(d => d.documentable?.status === 'DRAFT').length }}
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">
                    {{ devis.filter(d => d.documentable?.status === 'FINALIZED').length }}
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                    {{ devis.filter(d => d.documentable?.status === 'SENT').length }}
                  </span>
                </button>
                <button
                  ...
                  :class="[
                    'pb-3 text-sm font-bold transition-colors flex items-center gap-2',
                    selectedStatus === 'SIGNED' ? 'text-[#062121] border-b-2 border-[#C5F82A]' : 'text-gray-500 hover:text-gray-700',
                  ]"
                >
                  <i class="fas fa-file-signature"></i> Signés
                  <span class="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">
                    {{ devis.filter(d => d.documentable?.status === 'SIGNED').length }}
                  </span>
                </button>
                <button
                  ...
                  :class="[
                    'pb-3 text-sm font-bold transition-colors flex items-center gap-2',
                    selectedStatus === 'EXPIRED' ? 'text-[#062121] border-b-2 border-[#C5F82A]' : 'text-gray-500 hover:text-gray-700',
                  ]"
                >
                  <i class="fas fa-clock"></i> Expirés
                  <span class="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">
                    {{ devis.filter(d => d.documentable?.status === 'EXPIRED').length }}
                  </span>
                </button>
              </div>
              <button
          ...
          <div class="p-6 lg:p-8">
            ...
            <div v-else-if="filteredDevis.length === 0" class="text-center py-12">
              <i class="fas fa-calculator text-5xl text-gray-300 mb-4 block"></i>
              <p class="text-gray-500">
                {{ selectedStatus === 'all' ? 'Aucun devis créé pour le moment.' : 'Aucun devis avec ce statut.' }}
              </p>
              <button
                @click="createDevis"
              ...
              <table class="min-w-full">
                <thead>
                  <tr class="border-b border-gray-200">
                    <th class="px-4 py-3 text-left text-xs font-semibold text-[#062121] uppercase tracking-wider">N° Devis</th>
                ...
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="quote in filteredDevis" :key="quote.id" class="group hover:bg-white/50 transition-colors duration-200">
                    <td class="px-4 py-4 whitespace-nowrap">
                      <div class="text-sm font-semibold text-gray-900">#{{ quote.number || '—' }}</div>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-700">{{ quote.customer?.name || "Client inconnu" }}</div>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-600">{{ formatDate(quote.created_at) }}</div>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap text-right">
                      <div class="text-sm font-bold text-[#062121]">{{ formatCurrency(quote.total_ttc) }}</div>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap">
                      <span :class="['inline-flex px-2.5 py-1 text-xs font-semibold rounded-full', getStatusBadgeClass(quote.documentable?.status)]">
                        {{ getStatusText(quote.documentable?.status) }}
                      </span>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap text-right">
                      <div class="flex items-center justify-end gap-2">
                        <button @click="previewDevis(quote.id)" title="Aperçu" class="w-8 h-8 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-[#062121] transition-all duration-200">
          ...
          <div v-if="openDropdownId === quote.id">
            ...
            <template v-else-if="quote.documentable?.status === 'FINALIZED'">
              ...
              <button
                ...
                class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-3"
              >
                <i class="fas fa-file-invoice w-4 text-purple-500"></i> Créer facture d'acompte
              </button>
              <button
                @click="createPurchaseOrder(quote)"
                class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-3"
              >
                <i class="fas fa-clipboard-list w-4 text-blue-500"></i> Créer bon de commande
              </button>
              <button
                @click="createDeliveryNote(quote)"
                class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-3"
              >
                <i class="fas fa-truck w-4 text-green-500"></i> Créer bon de livraison
              </button>
              <button
                @click="convertToInvoice(quote)"
                class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-3"
              >
                <i class="fas fa-exchange-alt w-4 text-emerald-500"></i> Créer facture
              </button>
            </template>
          </div>
        </template>
