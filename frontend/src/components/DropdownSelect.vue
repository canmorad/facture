...
const currentLabel = computed(() => {
  const found = props.options.find(opt => opt[props.valueKey] === selectedOption.value);
...
const toggleDropdown = () => {
  if (props.options.length) {
    ...
    if (isOpen.value) {
      searchTerm.value = currentLabel.value !== props.placeholder ? currentLabel.value : '';
    }
  }
};
...
const handleInput = (event) => {
  ...
  const exactMatch = props.options.find(opt => String(opt[props.labelKey]) === inputValue);
  if (exactMatch) {
...
/>
    ...
    >
      <div
        ...
        :class="{ 'bg-[#C5F82A]/15 !text-[#062121] font-bold': opt[valueKey] === selectedOption }"
      >
      ...
      <div v-if="filteredOptions.length === 0" class="px-4 py-2.5 text-sm text-gray-500 italic text-center">
