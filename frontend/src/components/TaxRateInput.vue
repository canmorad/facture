...
const emit = defineEmits(["update:modelValue", "select-tax-rate"]);
...
const searchQuery = computed({
  ...
  set: (val) => emit("update:modelValue", parseFloat(val) || 0),
});

const filteredTaxRates = computed(() => {
  const query = searchQuery.value.toLowerCase().trim();
  if (!query) return props.taxRates;
  return props.taxRates.filter(
    (t) =>
...
const selectTaxRate = (taxRate) => {
  emit("select-tax-rate", taxRate);
  emit("update:modelValue", taxRate.rate);
  isOpen.value = false;
};
    ...
    <Teleport to="body" v-if="isOpen && !props.disabled">
      ...
      >
        ...
        <div class="max-h-60 overflow-y-auto">
          <div
            v-for="tax in filteredTaxRates"
            :key="tax.id"
            @mousedown.prevent="selectTaxRate(tax)"
          ...
          >
            <span class="flex-1 truncate">{{ tax.libelle }}</span>
            <span class="text-xs text-gray-500 font-mono ml-3 flex-shrink-0">
              {{ tax.rate }}%
            </span>
          </div>
          <div
            v-if="filteredTaxRates.length === 0"
